...
class DataClient(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.full_file_path = self.os_dir_suffix(
    ...
    def get_binance_pairs(self, **kwargs):
        ...
        if base_currencies and quote_currencies:
            input_pairs = [
                x+y for x in quote_currencies for y in base_currencies]
        ...
        if binance_pairs:
            ...
            raise ValueError(
                'Invalid Input: Binance returned no matching currency pairs.')
    ...
    def process_csv_dates(self, pair, interval, **kwargs):
        ...
        if not start_date and not first_csv_date and not end_date:
            ...
            if end_date < earliest_timestamp:
                raise ValueError(
                    'Invalid Date Range: end date is prior to Binance open date of 07/14/2017')
            date_ranges.append([earliest_timestamp, end_date])
            ...
            if end_date < start_date:
                ...
                date_ranges.append(
                    [start_date, first_csv_date-datetime.timedelta(days=1)])
                date_ranges.append([last_csv_date, end_date])
                past_csv_files = sorted(os.listdir(csv_dir))
                os.remove(''.join([csv_dir, past_csv_files[-1]]))
            elif start_date >= first_csv_date and end_date >= last_csv_date:
                ...
                return None
        dates = list(chain.from_iterable([[date for date in rrule(
            DAILY, dtstart=x[0], until=x[1]+datetime.timedelta(days=1))] for x in date_ranges]))
        return dates
        ...
        if output:
            ...
            for x in output:
                try:
                    ...
                    raise ValueError(
                        'Invalid Output Field: valid fields include opened,open,high,low,close,volume,closed,quote_volume,total_trades,taker_buy_base_volume,taker_buy_quote_volume')
    ...
    def create_csv_directories(self, pair_list, kline_interval, historical_price_data_directory=None):
        if not historical_price_data_directory:
            historical_price_data_directory = '{}historical_price_data'.format(
                self.full_file_path)
        try:
            os.makedirs(historical_price_data_directory)
        except OSError:
            pass
        kline_interval_directory = ''.join([self.os_dir_suffix(
            historical_price_data_directory), '{}_data'.format(kline_interval)])
        try:
        ...
        for x, p in enumerate(pair_list):
            pair_directory = ''.join(
                [self.os_dir_suffix(kline_interval_directory), '{}'.format(str(p))])
            try:
                os.makedirs(pair_directory)
            except OSError:
                pass
            individual_csvs_directory = ''.join(
                [self.os_dir_suffix(pair_directory), 'individual_csvs'])
            try:
    ...
    def past_csv_check(self, kline_interval_directory, pair):
        individual_csvs_directory = ''.join([self.os_dir_suffix(
            kline_interval_directory), self.os_dir_suffix(pair), self.os_dir_suffix('individual_csvs')])
        past_csv_files = sorted(os.listdir(individual_csvs_directory))
        if past_csv_files:
            self.csv_dates['first'] = datetime.datetime.strptime(
                past_csv_files[0].replace('.csv', ''), '%Y-%m-%d')
            self.csv_dates['last'] = datetime.datetime.strptime(
                past_csv_files[-1].replace('.csv', ''), '%Y-%m-%d')
    ...
    def concatenate_csvs(self, csv_file_info):
        for x, file_info in enumerate(csv_file_info):
            ...
            concat_csv_path = self.os_dir_suffix(os.path.join(
                *(output_path.rsplit(os.path.sep, 2)[:-2])))
            individual_csv_files = sorted(
                [f for f in os.listdir(output_path) if f.endswith('.csv')])
            old_concat_csvs = [f for f in os.listdir(
                concat_csv_path) if f.endswith('.csv')]
            concat_csv = '{}.csv'.format(pair)
            if concat_csv in old_concat_csvs:
                old_concat_csvs_path = '{}{}'.format(
                    concat_csv_path, self.os_dir_suffix('old_concatenated_csvs'))
                try:
                    os.makedirs(old_concat_csvs_path)
                except OSError:
                    pass
                shutil.move('{}{}'.format(concat_csv_path, concat_csv), '{}{}'.format(
                    self.os_dir_suffix(old_concat_csvs_path), concat_csv))
    ...
    def kline_to_csv(self, pair, start_date, end_date, kline_interval_directory, interval, csv_file_info):
        ...
        date_range = self.process_csv_dates(
            pair, interval, start_date=start_date, end_date=end_date)
        if not date_range:
            return
        output_path = ''.join([self.os_dir_suffix(kline_interval_directory), self.os_dir_suffix(
            pair), self.os_dir_suffix('individual_csvs')])
            ...
            if date != date_range[-1]:
                if date_range[x+1] != date+datetime.timedelta(days=1):
                    ...
                    if self.progress_statements == True:
                        print('currency pair: {} start: {} end: {}'.format(
                            pair, start, end))
                    ...
                    if klines:
                        ...
                        results_csv = '{}{}{}.csv'.format(
                            output_path, csv_month, csv_day)
                        ...
                        for y, kline in enumerate(klines):
                            ...
                            self.opened = datetime.datetime.utcfromtimestamp(
                                float(self.open_timestamp)/1000).strftime('%Y-%m-%d %H:%M:%S')
                            self.closed = datetime.datetime.utcfromtimestamp(
                                float(self.close_timestamp)/1000).strftime('%Y-%m-%d %H:%M:%S')
                            csv_fields = [getattr(self, field)
    ...
    def kline_data(self, pair_list, interval, **kwargs):
        ...
        valid_kline_intervals = ['1m', '3m', '5m', '15m',
                                 '30m', '1h', '2h', '4h', '6h', '8h', '12h',"24h"]
        if interval not in set(valid_kline_intervals):
            raise ValueError(
                'Invalid Interval: Kline interval should be one of the following - {}'.format(','.join(valid_kline_intervals)))
        ...
        if storage_method.lower() == 'csv':
            ...
            data = pool.starmap(self.kline_to_csv, zip(pair, re(start_date), re(
                end_date), re(kline_interval_directory), re(interval), re(csv_file_info)))
            pool.close()
            pool.join()
